...
-   ¿Existe una asoación entre el peso_nacer y la edad_madre.?
...
```{r}
plot(peso_nacer ~ edad_madre , data = bajo_peso,
     col = "gray",
     ylab = "peso_nacer",
     xlab = "edad_madre",
     las = 1,
...
abline(lm(peso_nacer ~ edad_madre , data = bajo_peso), lwd = 2, col = "darkred")
```
...
```{r}
modelo_ejemplo = lm(peso_nacer ~ edad_madre, data = bajo_peso)
```
...
tabla_reporte <- modelo_ejemplo |> 
  tbl_regression(intercept = T,
                 ...
                 label        = list(edad_madre ~ "edad de la madre")) |>
  modify_caption("Regresión del peso al nacer en función a la edad de la madre")
...
```{r}
group_by(bajo_peso, raza) |> 
  summarise(
    count = n(),
    mean = mean(peso_madre, na.rm = TRUE),
    sd = sd(peso_madre, na.rm = TRUE)
  )
```
...
```{r}
bajo_peso |>  
  filter(!is.na(raza)& !is.na(peso_madre))  |> 
  ggplot(aes(y = peso_madre, x = raza)) +
  geom_boxplot() +
  labs(y = "peso de la madre", x = "raza")
```
...
bajo_peso |> 
  filter(raza == "Blanca") |> 
  summarise(shapiro = list(shapiro.test(peso_madre))) |> 
  pull(shapiro)
```

```{r}
bajo_peso |> 
  filter(raza == "Afroamericana") |> 
  summarise(shapiro = list(shapiro.test(peso_madre))) |> 
  pull(shapiro)
```
...
```{r}
var.test(peso_madre ~ raza, data = bajo_peso)
```
...
El valor p de la prueba es 0.008667, lo cual es menor que el nivel de significancia α = 0.05. Por lo tanto, podemos concluir que el peso al nacer de RN en madres que fuman es significativamente diferente de el peso al nacer de RN en madres que no fuman.
...
```{r}
group_by(bajo_peso, hipertension) |> 
  summarise(
    count = n(),
    mean = mean(peso_nacer, na.rm = TRUE),
    sd = sd(peso_nacer, na.rm = TRUE),
    min = min(peso_nacer, na.rm = TRUE),
    max = max(peso_nacer, na.rm = TRUE)
  )
```
...
```{r}
bajo_peso |>  
  filter(!is.na(hipertension)& !is.na(peso_nacer))  |> 
  ggplot(aes(y = peso_nacer, x = hipertension)) +
  geom_boxplot() +
  labs(y = "Peso al nacer", x = "raza")
```
...
```{r}
bajo_peso <- bajo_peso |>
    mutate(hipertension = as.factor(hipertension))
```
...
bajo_peso |> 
  filter(fuma == "Sí") |> 
  summarise(shapiro = list(shapiro.test(peso_nacer))) |> 
  pull(shapiro)
```
...
```{r}
res_anova = aov(peso_nacer ~ fuma, data = bajo_peso)
```
